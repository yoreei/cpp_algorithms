...
void bfsAlgorithm(const Graph& graph, int startNode, vector<int>& output) {
    ...
    while (!q.empty())
    {
        ...
        for (auto child_it = graph.at(current).cbegin(); child_it != graph.at(current).cend(); ++child_it)
        {
            if (visited.find(*child_it) == visited.cend())
            {
                q.push(*child_it);
...
void bfsAlgorithmV2(const Graph& graph, int startValue, vector<int>& output)
{
    ...
    {
        std::queue<NodeV2*> q{}; // make queue<int>
        // std::bitset<INT32_MAX> visited {}; // STACK OVERFLOWS!
        //q.push(root);
        //visited[startValue] = root;
